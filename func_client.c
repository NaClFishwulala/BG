...
//	unsigned char destaddr[ETH_ALEN] = { 0xFF,0xFF,0xFF,0xFF,0xFF,0xFF };
	...
	if (unlikely(CHECKSUM_WORKMODE == 1)) {
		...
		printk(KERN_INFO "tx: no CHECKSUM\n");
	}
	...
	if(unlikely(ENCRYPT_WORKMODE == 1)){
		...
		printk(KERN_INFO "tx: no ENCRYPT\n");
	}
	...
	skb->dev = ens_dev;
	dev_queue_xmit(skb);
	printk(KERN_INFO "tx: send one packet to dev:%s\n", skb->dev->name);
	...
	struct packet_type* ptype, struct net_device* orig_dev)
{
	...
	netif_rx(sb);
	printk(KERN_INFO "rx:send one packet from dev:%s\n", sb->dev);
	return 0;
}
